...
export class GenDashCard extends LitElement implements LovelaceCard {
        ...
        <div class="right-pane">
          <div class="preview-container">
            ...
            <div class="preview-content">
              ${this._generatedYaml ? this._renderPreview() : html`
                <div class="empty-state">
                  <ha-icon icon="mdi:view-dashboard-outline"></ha-icon>
                  <div>Generated dashboard will appear here</div>
                </div>
              `}
            </div>
          </div>
  ...
  private async _getConfigEntryId(): Promise<string | null> {
    ...
    try {
     ...
     const genDashEntries = (response as { domain: string; entry_id: string }[]).filter(
       (entry) => entry.domain === 'gen_dash'
     );
  ...
  private _renderPreview() {
    try {
      ...
      const yaml = (window as any).jsyaml.load(this._generatedYaml);
      if (typeof yaml !== 'object' || yaml === null) {
        return this._renderYaml();
      }

      const cards = yaml.cards || [yaml];
      return html`
        ${cards.map((cardConfig: LovelaceCardConfig) => {
